...
\end{align}
An important aspect of implementing \eqref{eq:siamstar}--\eqref{eq:xjsyks}, because we use the piecewise-defined FE flux $\bq^h(x,y)$, is that in our indexing we have
\begin{align}
&\qquad\qquad (x_j^s,y_k^s) \in \square_{u,v} \label{eq:quadpointinelement} \\
&\iff \quad u = j\!-\!\eta(x_j^s\!-\!x_j) \text{ and } v\!=\!k-\eta(y_k^s\!-\!y_k), \notag
\end{align}
where $\eta(t)$ is the Heaviside step function, $\eta(t)=0$ if $t<0$ and $\eta(t)=1$ if $t>0$ (Figure \ref{fig:improvequadrature}).

\begin{figure}[ht]
\begin{center}
\input{fig3.tikz}
\end{center}
\caption{For equation \eqref{eq:siamstar} we evaluate $\bq^h(x,y)$ at eight quadrature points (numbered circles) along $\partial V_{j,k}$ (dashed).  Note points $(x_j^0,y_k^0)$ and $(x_j^7,y_k^7)$ are the points used in \eqref{eq:starbreakfirst}.}
...
Our proposed improvements merely allow us to write down a system of nonlinear algebraic equations.  It remains to describe the numerical solution of this system.  This is nontrivial because we solve for steady state directly, and because numerically-locating the free boundary (ice sheet margin) is an integral part of the problem.

We suppose that the structured grid has periodic boundary conditions (b.c.s) in each direction, appropriate to a whole ice sheet model.  Each equation in the \Mstar scheme is simply \eqref{eq:siafve} with quadrature \eqref{eq:starbreakfirst} and upwind modification \eqref{eq:upwindschemex}.  The resulting highly-nonlinear system of $N=N_x N_y$ equations
\begin{equation}
