...
\end{center}
\caption{{\large \textbf{a.}}~A structured grid as a FD grid with regular (dots) and staggered (triangles) locations.  {\large \textbf{b.}}~The same grid as a FE grid with rectangular elements (solid), nodes (dots), and dual rectangular control volumes (dashed).}
...
\end{equation}
where $\partial V$ denotes the boundary of $V$.  This is simply the integral form of \eqref{eq:siasteady}.
...
So consider the structured grid of rectangles with dimensions $\Delta x,\Delta y$ shown in Figure \ref{fig:fdfemgrids}b.  The rectangle $\square_{j,k}$ has lower-left corner at $(x_j,y_k)$.  These rectangles are $Q^1$ finite elements when associated with bilinear functions.  A basis of bilinear functions on $\square_{j,k}$ is
\begin{equation}
