...
The result of this strategy is shown in Figure \ref{fig:grnwinset}.  In particular, this result came from a nearly-converged steady-state computation ($\eps_{11}$ level) on the most-smoothed bed, then running the five-step bed resolving process using very short ($\sim10^{-3}$ a) backward Euler time steps, and then 30 model years of additional backward Euler time steps where the last 20 years used one week time steps ($0.02$ a).  Though the average diffusivity over the whole ice sheet was small ($D\approx 0.5\,\text{m}^2\,\text{s}^{-1}$), there are maximum values in the interiors of highly-resolved outlet glaciers of $D\approx 7\times 10^3$.  This highly-variable $D=D(x,y)$ map at steady-state apparently represents a barrier to the convergence of the continuation/Newton scheme for steady-state computations.  Presumably it is also a barrier for the convergence of longer time steps, but this was not studied quantitatively.
...
We do not claim the model in question---namely isothermal SIA---is sufficiently-complete to represent the all of the important physics.  (Shallow hybrid models are, however, capable of very-high-quality match to the observations \citep{Aschwandenetal2015}.)  The result in Figure \ref{fig:grnwinset} has modeled volume $3.48\times 10^6\,\text{km}^3$ compared to observed value $2.95\times 10^6\,\text{km}^3$, and the average absolute thickness error was 139 m.  This error is dominated by large errors from mis-locating the ice margin in fjord-like coastal areas.


\section{Discussion and Conclusion} \label{sec:conclusion}

The problem solved in this paper is fundamentally different from most prior ice sheet modeling.  We compute the steady-state geometry and extent of an ice sheet as a function of two data sets, namely the bed topography and surface mass balance.  Though this function has not been proved to be well-defined, even in the elevation-independent surface mass balance cases here---compare the elevation-dependent case studied by \cite{Jouvetetal2011}---there is some theoretical support for well-posedness \citep{JouvetBueler2012} and nothing seen in our calculations suggests otherwise.
