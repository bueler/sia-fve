...
Denote the local coordinates of the quadrature point $(x_j^s,y_k^s)$ on the element $\square_{u,v}$ by $\xi^s = (x_j^s-x_u)/\Delta x$ and $\eta^s = (y_k^s-y_v)/\Delta y$.  Note that upwinding determines an additional evaluation point, whose local coordinates are denoted $\uppoint$, for each quadrature point---see Figure \ref{fig:upwindterm}.  
