...
\end{enumerate}
to create the new \Mstar scheme.  Then, because the constraint of nonnegative thickness makes this a free-boundary problem, we put it in complementarity form and apply a parallel Newton solver designed for such problems.
...
Convergence of the Newton iteration in the presence of finely-resolved, and thus very rough, bed is not guaranteed.  The highly-variable values of diffusivity $D$ from the result in Figure \ref{fig:grnwinset} (not shown) apparently forms part of the barrier to full convergence of the continuation/Newton scheme for such steady-state computations.  The convergence of longer time steps on rough beds is also limited, but this was not studied quantitatively.  A complete and/or precise understanding of the failure of convergence of the Newton iteration on rough beds is a topic for further research.
