...
\end{center}
\caption{\textbf{a.}~A structured FD grid with regular points (dots) and staggered points (triangles).  \textbf{b.}~The same grid as an FVE grid with rectangular elements $\square_{j,k}$ (solid), nodes (dots), a dual rectangular control volume $V_{j,k}$ (dashed), and flux-evaluation locations (circles).  Corners $\ell=0,1,2,3$ of the element are where each basis function $\chi_\ell$ equals one.}
