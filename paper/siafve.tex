...
In this section we first apply our method in two examples where exact solutions are known.  Our method consists of the \Mstar scheme \eqref{eq:siamstar} to set up the discrete system \eqref{eq:nonlincomplementarity}, on the one hand, and regularization \eqref{eq:nonlinregularization}, continuation scheme \eqref{eq:continuationND}, and the constraint-adapted Newton solvers in PETSc, on the other.  We use the verification cases to address both the convergence of the discrete solution to the continuum solution, under grid refinement, and the convergence of the Newton iteration on the discrete system.  Then we apply the method in an extended Greenland ice sheet example using high-resolution bedrock topography.

An angularly-symmetric exact solution is constructed by \cite{Bueler2003} in the flat bed case; see also subsection 5.3 of \cite{vanderVeen2013}.  This ``dome'' exact solution, with parameters suitable for a medium-sized ice sheet, is shown in Figure \ref{fig:domeprofile}.  FIXME A relatively coarse-grid 500 m application of the \Mstar method is compared and is obviously very close, with the only visible error 
...
FIXME: Figure \ref{fig:newtonconv} show evidence of quadratic convergence; compare \citep{BrownSmithAhmadia2013}

\begin{figure}[ht]
\onecol{newtonconv.pdf}
\caption{At each stage $\eps_i$ of the continuation scheme, the Newton iteration shows the characteristic curved drop of quadratic convergence (residual norm vs iteration number on semilog axes).  The SIA problem itself has $\eps_{12}=0$ (last curve).}
