...
In this section we first apply our method in two examples where exact solutions are known.  On the one hand we use the \Mstar scheme \eqref{eq:siamstar}, which we compare to the classical Mahaffy scheme, when generating a discrete system \eqref{eq:nonlincomplementarity}.  On the other hand, our method has a regularization \eqref{eq:nonlinregularization}, a continuation scheme \eqref{eq:continuationND}, and a constraint-adapted Newton solver from PETSc to solve the system.  Thus we use the exact solutions, i.e.~verification cases, to evaluate the convergence of the discrete solution to the continuum solution, under grid refinement.  And we use a verification case to evaluate the convergence of the Newton iteration.  After that we apply the method in an extended Greenland ice sheet example using high-resolution bedrock topography.

An angularly-symmetric steady state exact solution \citep{Bueler2003} exists in the flat bed case; see also subsection 5.3 of \cite{vanderVeen2013}.  This ``dome'' exact solution, with parameters suitable for a medium-sized ice sheet, is shown in Figure \ref{fig:domeprofile}.  A relatively coarse 25 km application of the \Mstar method is compared.  The numerical result is very close, with the only barely-visible error at the last positive-thickness grid point.  The inset, which shows results for the same grid and a refined grid, illustrates the consequence of large gradient at the margin, namely large errors which go away slowly under grid refinement \citep{Bueleretal2005}.

\begin{figure}[ht]
\onecol{domeprofile.pdf}
\caption{Result from \Mstar method compared to the dome exact solution ($\Delta x=25$ km), with a detail of the margin showing result from 25 km (dots) and 12.5 km (diamonds) grids.}
\label{fig:domeprofile}
\end{figure}

With the same exact solution we can measure the maximum and average thickness error from the classical Mahaffy and \Mstar methods.  As shown in Figure \ref{fig:domeverif}, both methods show very slow convergence of maximum error, for the reason we have just stated.  The rate of convergence of the average error (dotted lines) is distinctly better for \Mstar, but in fact the Newton method fails to converge for all classical Mahaffy runs except the coarsest.  This distinctly-improved convergence of the \Mstar method, which only differs from the classical method by improved quadrature in this flat bed case, is seen in all cases tried.  Though the \Mstar method fails to converge on the two finest grids, these cases converge if the problem is changed to a backward Euler step \eqref{eq:backwardeuler} of duration $1000.0$ years. % FIXME: confirmed for level 5; check result from level 6

\begin{figure}[ht]
\onecol{domeverif.pdf}
\caption{Average and maximum error under grid refinement using the dome exact solution, for the \Mstar (stars) and classical Mahaffy (squares) methods.  Gray circles indicate runs where the Newton method diverged before the last $\eps=0$ stage of the continuation sequence \eqref{eq:continuationseq}.}
\label{fig:domeverif}
\end{figure}

Figure \ref{fig:newtonconv} shows clear evidence of quadratic, or at least superlinear, convergence from the Newton solver.  At each stage $\eps_i$ of the continuation scheme, the computed residual shows a characteristic curved drop on log-residual-norm versus iteration number axes \citep{Kelley2003}.  The SIA problem itself is the last $\eps_{12}=0$ curve.

\begin{figure}[ht]
\onecol{newtonconv.pdf}
\caption{Clear evidence of superlinear convergence at each continuation state appears when the residual norm is plotted.}
...
As we have already noted, the flat bed case has been well-studied.  FIXME: we try bedrock step solution from \cite{JaroschSchoofAnslow2013}, with result in Figure \ref{fig:bedstepprofiles}  FIXME: verification on bedrock step solution shows no convergence at all in maximum and average errors, but like \cite{JaroschSchoofAnslow2013} we can show relative volume error; Table \ref{tab:bedstepvol} shows $100 (V_{\text{numerical}} - V_{\text{exact}}) / V_{\text{exact}}$; \cite{JaroschSchoofAnslow2013} have explicit implementations which may be more important than their use of true Mahaffy
...
FIXME: we have been basically flux-agnostic, and the same basic FVE works as long as there is a way of computing $\bq^h(x_j^s,y_k^s)$ from the current $\bH$.  main issue is extending velocity, thus flux, to cover area that might be new support of $H^h$.  how to modify our methods to include sliding and flow models based on membrane stress balances.
