...
\abstract{The isothermal, non-sliding shallow ice approximation, combined with mass conservation, is a fundamental model for ice sheet and glacier flow.  It determines the ice geometry by the solution of a free-boundary problem.  We solve the steady state form of this problem directly, without time-stepping, thereby demonstrating a fully-implicit scheme with no stability restrictions.  We first re-interpret the \cite{Mahaffy1976} finite difference method as a ``finite volume element'' (FVE) scheme.  Such a scheme has both an everywhere-defined numerical solution and a flux integral form of the conservation equation.  On the basis of this re-interpretation we build an improved scheme which has both better quadrature in the integral and upwinding on that part of the flux which is proportional to the bed gradient.  The discrete equations are then solved by parallel Newton schemes which respect the constraint that ice thickness is nonnegative.  The results show superior accuracy to any published results for both a flat bed and a bedrock-step exact solution.  In these verification cases convergence of the Newton method converges without exception.  We then apply the improved scheme at various high resolutions, including sub-1km grids, to model the steady-state geometry of the Greenland ice sheet, using only bedrock elevation and present-day surface mass balance as input data.  While convergence problems arise with highly-resolved bed topography in the Greenland applications, robustness is completely-restored by switching the problem to taking long (10 to 1000 year) fully-implicit time steps.}
...
FIXME: like \cite{JouvetBueler2012} result but much higher res and replacing fixed-point iteration by continuation 
