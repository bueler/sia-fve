...
While we have demonstrated its effectiveness on verification cases, the success of our method depends on robust convergence when the data of the problem, especially the bed topography $b$, are realistically non-smooth.  To test robustness and scalability to high-resolution and many processors, we use two Greenland ice sheet bed topography data sets of different degrees of smoothness.  We show convergence on rough beds at sub-1 km resolution if care is taken in constructing the iteration.

The smoother bed is the older BEDMAP1 topography on a 5 km data grid \citep{Bamberetal2001}; we call this bed data ``BM1''.  This data, and a gridded model for present-day surface mass balance \citep{Ettemaetal2009} are included in the 5 km SeaRISE data \citep{Bindschadleretal2013}.  All of our experiments use the \cite{Ettemaetal2009} surface mass balance, or a reprojected and interpolated version thereof.  The much finer resolution, and thus much rougher, bed data are on a 150 m grid from \cite{Morlighemetal2014}, generated by mass-conservation methods from additional ice-penetrating radar flightlines, i.e.~since 2001, and based on surface velocity measurements; we call this bed data ``MCB''.

Considering the BM1 bed first, we compute the \Mstar steady-state solution on 5000, 2500, 1667, 1250, 1000, and 625 meter grids.  In all cases except the first we refine the 5 km grid using bilinear interpolation, so these beds effectively become smoother under grid refinement.  Figure \ref{fig:grnrobusteps} shows that in each case the Newton solver does not converge at the $\eps_{12}=0$ level, but that the better levels ($\eps_9$--$\eps_{11}$) of the continuation scheme are reached in all cases.

\begin{figure}[ht]
\onecol{grnrobusteps.pdf}
\caption{Smallest successful value of $\eps_i$ for which the Newton iteration converges on the steady-state problem; lower is better.  Different bed data sources (BM1 or MCB) and complementarity solvers (rs or ss) are compared.}
\label{fig:grnrobusteps}
\end{figure}

The runs using the MCB data were on 4500, 3000, 1500, 1200, 900, and 600 meter grids.  The bed elevations were averaged from the 150 m given data, from 30$\times$30 blocks for the 9 km grid to 4$\times$4 blocks for the 600 m grid.  (We did not attempt the computation at 150 m.)  Because this process reveals more and more detail under grid refinement, in this case the bed effectively becomes rougher under grid refinement.  In this case, except on the coarsest and most-averaged grid, the continuation scheme fails to generate convergent Newton solves beyond the middle stages $\eps_4$--$\eps_6$.  It is clear that highly-resolved bed, which causes large and irregular values of $D$ and $\bW$ to arise from formulas \eqref{eq:siafluxdiffusion} and \eqref{eq:siaWdefine}, limits the success of the Newton iteration.

% ./robustfigs.py --ratios study.robust
Because of a lack of theoretical guidance on the choice of complementarity problem solver in our context (compare \cite{BensonMunson2006,JouvetGraeser2013}), and because the two available choices gave comparable results on verification cases, we also compared the reduced-space (``rs'') and semi-smooth (``ss'') methods for solving complementarity problem \eqref{eq:nonlincomplementarity} in these realistic Greenland ice sheet cases.  From Figure \ref{fig:grnrobusteps} it is clear that the convergence of the two methods is comparable, with the rs method possibly having an edge.  However, because the ss method is, on average over all the runs, a factor of three slower, it is easy to recommend the rs solver for these problems.

FIXME  not prepared to give up entirely.  approach problem by solving steady state on 750 m grid but with highly-averaged bed (onto 9000 m); this succeeds to $\eps_{11}$.  then do time-stepping on 750 m grids with less and less averaging of the bed till it is also 750 m.  result in Figure \ref{fig:grnwinset}
