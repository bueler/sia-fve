...
This form of upwinding is most important at locations of low regularity of the solution.  We will see in verification (Results section) that it achieves higher solution accuracy than either the apparently second-order Mahaffy scheme or the reported results for the higher-resolution explicit scheme of \citep{JaroschSchoofAnslow2013} at the location of the bedrock step.

We call the method which combines both improve-ments---i.e.~better quadrature and a bit of upwinding---the ``\Mstar'' scheme.  The convergence of the Newton method for solving the equations from this scheme is greatly improved compared to the classical Mahaffy scheme (Results section).
...
\caption{Our method computes high-resolution ice sheet margins at the locations uniquely-determined by the surface mass balance, bedrock topography, and the simplified-dynamics SIA model.  Here: 600 m model resolution and MCB bed topography data.}
