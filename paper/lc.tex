...
\section{Well-posedness for the single time-step problem} \label{sec:wellposed}  Our goal in this section is to determine which properties of the flux $\bQ_n$ will lead to the problem \eqref{eq:theVI} being well-posed.  Techniques for proving well-posedness of time-independent variational inequalities are relatively well-established for linear and even nonlinear elliptic problems which correspond to monotonic forms \cite{KinderlehrerStampacchia}, and we look at this case first (subsection \ref{subsec:mono}).  Because fluid layer conservation equations can use a variety of non-parabolic and non-linear flux forms $\bq$, and because we want to allow for a variety of time-discretizations, we then consider a diverse range of variational inequality problems in subsections \ref{subsec:semilinear}--\ref{subsec:explicit}, and we get a range of generally less-complete results compare to the parabolic, monotonic case in \ref{subsec:mono}.

In this section we denote the pairing $L^q(\Omega;\RR^d) \times L^p(\Omega;\RR^d)$, where $\frac{1}{p}+\frac{1}{q}=1$, by the same symbols $\ip{\cdot}{\cdot}$ used for the pairing of the dual $\mathcal{X}'$ with the Banach space $\mathcal{X}$ itself (definition \ref{def:spaces}).  No confusion should result.

FIXME: consider $L^1$-contractivity in general?

\subsection{Case I: Monotonicity and quasilinear parabolic fluxes} \label{subsec:mono}  We start by defining terms.  Recall that $\mathcal{X}$ is a Banach space and $\mathcal{K}$ is a closed, convex subset of $\mathcal{X}$.  We define a mapping $A : \mathcal{K} \to \mathcal{X}'$ to be \emph{monotone} \cite{KinderlehrerStampacchia} if
\begin{equation}
...
\begin{equation}
\Delta t\,\ip{\bQ_n(\grad u,u,x) - \bQ_n(\grad v,v,x)}{\grad(u-v)} \le C \|u-v\|_{L^2}^2 \label{eq:Qnmonotone}
\end{equation}
...
\begin{equation}
\Delta t\,\ip{\bQ_n(\grad u,u,x) - \bQ_n(\grad v,v,x)}{\grad(u-v)} \le -\eps \|u-v\|^r \label{eq:Qncoercive}
\end{equation}
for all $u,v \in \mathcal{K}$.
\end{lemma}

Note that the norm on the right side of \eqref{eq:Qncoercive} is for $W_0^{1,p}(\Omega)$.  Observe that \eqref{eq:Qncoercive} implies \eqref{eq:Qnmonotone} with $C=0$, so \eqref{eq:Qncoercive} implies strict monotonicity for $A_n$.  Also, \eqref{eq:Qnmonotone} is necessary and sufficient for monotonicity of $A_n$, while clearly \eqref{eq:Qncoercive} is merely sufficient for coercivity.  For example, if right side of \eqref{eq:Qncoercive} were ``$-\eps \|u-v\| \log \|u-v\|$'' then $A_n$ would also be coercive.
...
We can directly apply these well-posedness results to some non-linear parabolic cases.  Consider the $p$-Laplacian \cite{Evans} flux
\begin{equation}
  \bQ_n(\grad u,u,x) = - k |\grad u|^{p-2} \grad u \label{eq:plapflux}
\end{equation}
...
\subsection{Case II: Other parabolic fluxes by semi-linear techniques} \label{subsec:semilinear}  Now we consider the porous medium  \cite{Evans} flux
\begin{equation}
  \bQ_n(\grad u,u,x) = - k \grad(u^\gamma) \label{eq:porousflux}
\end{equation}
where $k>0$ and $\gamma > 0$.  Leaving the relevant Sobolev space undetermined for a moment, let
	$$w = (u_n)^\gamma$$
...
\subsection{Case III: Hyperbolic fluxes} \label{subsec:hyperbolic}  FIXME  We are certainly not interested in restricting to parabolic problems.  The simplest hyperbolic problems involve a flux which comes from considering a layer which is transported by a differentiable velocity field $\bX \in W^{1,\infty}(\Omega;\RR^d)$, so that
\begin{equation}
...
\begin{equation}
\Delta t\,\ip{\bQ_n(\grad u,u,x) - \bQ_n(\grad v,v,x)}{\grad u - \grad v} \le 0,
\end{equation}
...
\subsection{Case IV: Explicit time-steps} \label{subsec:explicit}  FIXME  Presumably go to $L^2(\Omega)$ as Banach space and use truncation result for the variational inequality, i.e.~from page 27 of \cite{KinderlehrerStampacchia}

FIXME then add example where $\bq$ comes from integrating over $\Omega$ with a kernel


\section{Time-series corrections when reporting on mass conservation}  \label{sec:timeseries}
