...
Techniques for proving well-posedness, i.e.~existence and uniqueness, of time-independent variational inequalities in Banach spaces are relatively well-established for certain linear and some nonlinear elliptic problems.  We recall some of these tools, for monotone variational inequalities \cite{KinderlehrerStampacchia}, in subsection \ref{subsec:mono}.  Then in the next four subsections we prove existence and uniqueness of problem \eqref{eq:theVI} for these flux cases:
\begin{itemize}
...
\subsection{Monotonicity and sufficient conditions on flux for well-posedness} \label{subsec:mono}  Recall that $\mathcal{X}$ is a Banach space and $\mathcal{K}$ is a closed, convex subset of $\mathcal{X}$.  We define a mapping $A : \mathcal{K} \to \mathcal{X}'$ to be \emph{monotone} \cite{KinderlehrerStampacchia} if
\begin{equation}
...
\end{align}
The following lemma uses the fact that $W^{1,p}(\Omega) \subset L^2(\Omega)$ if either $p>d$ or $d\le 2$; see Sobolev's lemma, theorems 5.6.2 and 5.6.5, in \cite{Evans}.
...
\medskip
Corollary III.1.8 of \cite{KinderlehrerStampacchia} now gives the following sufficient-conditions theorem.
...
\subsection{$p$-Laplacian fluxes} \label{subsec:plap}  We can immediately apply lemma \ref{lem:monotonecoercive} and theorem \ref{thm:monowellposed} to show well-posedness for some linear and non-linear parabolic cases, i.e.~where $\bq$ has the direction of $-\grad u$.  Consider the $p$-Laplacian \cite{Evans} flux
\begin{equation}
...
This result can be exploited in a couple of ways.  If the vector field is divergent, i.e.~if $\Div\bX\ge 0$, then clearly \eqref{eq:Qnmonotone} applies with $C=0$, and so $A_n$ is strictly monotone.  Otherwise, \eqref{eq:Qnmonotone} applies with $C = \frac{1}{2}\Delta t\,\|\Div\bX\|_{L^\infty(\Omega)}$, so it follows that $A_n$ is monotone if $C\le 1$, and strictly monotone if $C<1$.  Note that any bound on this $C$ can be regarded as a CFL-type condition \cite{LeVeque}, though here the bound is not on the velocity magnitude but rather on a measure of the size of that part of the velocity field that is not ($\RR^d$-)volume-preserving.
...
\end{equation}
where $k>0$, $r\ge 0$, and $1<p<\infty$.  Of course this includes, as the $r=0$ case, the $p$-Laplacian \eqref{eq:plapflux}.  However, at the opposite extreme it arises in the porous medium equation \cite{Vazquez2007}, where $p=2$ and $r=\gamma-1$ so that the flux is frequently written $- \tilde k \grad(u^\gamma)$.  However, both powers are nontrivial for the diffusive shallow water equations \cite{AlonsoSantillanaDawson}, where $1<r<2$ and $1<p\le 2$, and for the shallow ice approximation \cite{BLKCB,CDDSV,JouvetBueler2012}, where $r=n+2$ and $p=n+1$ for an experimentally-determined exponent $1 < n \lesssim 4$ \cite{GoldsbyKohlstedt2001}.  Thus
...
The scalar normal flux across the edge $(j,k)$, outward from cell $j$, is denoted $Q_n^{(j,k)}$.  We assume that the fluxes $Q_n^{(j,k)}$ are approximated by some scheme, based on some subsets of the values $\{u_n^j\}$, so that two things are true:\begin{enumerate}
\item Edges with zero thickness on both sides have zero flux:
\begin{equation}
  Q_n^{(j,k)} = 0  \qquad \text{ if } u_n^j = 0 \text{ and } u_n^k = 0.  \label{eq:fvQiszero}
\end{equation}
(Compare assumption \eqref{eq:Qiszero} in the continuous-space case.)
\item There is local conservation between any two adjacent cells which are fluid-filled:
\begin{equation}
  Q_n^{(k,j)}=-Q_n^{(j,k)}  \qquad \text{ if } u_n^j u_n^k > 0.  \label{eq:fvlocalconservation}
\end{equation}
\end{enumerate}

We also assume that $F_n^j$ is a representative source term for the $j$th cell, so that
	$$F_n^j \approx \frac{1}{a_j} \int_{\omega_j} F_n(x)\,dx.$$
It is computed in some way from the geometry of cell $j$ and from values $\{u_n^j\}$ as necessary; the details do not matter here.

Now recall that, at least heuristically, the divergence is the limit of line (surface) integrals,
	$$\Div \bX(x) = \lim_{|R|\to 0} \frac{1}{|R|} \int_{\partial R} \bQ_n\cdot \bn\,ds,$$
where $R\subset \RR^d$ denotes a region with Lipshitz boundary, enclosing $x$, with outward unit normal vector $\bn$ and boundary length (area) element $ds$.  Thus, regarding the finite-volume discretization, we assume that the strong form single-time-step problem \eqref{eq:semimassconserve} is approximated by
\begin{equation}
\frac{u_n^j - u_{n-1}^j}{\Delta t} + \frac{1}{a_j} \sum_{k\in \mathcal{E}_j} Q_n^{(j,k)} \ell_{(j,k)} = F_n^j \label{eq:fvmassconserve}
\end{equation}

Many schemes can be given interpretations \eqref{eq:fvthickness} and \eqref{eq:fvmassconserve}.  The schemes includestructured-grid (i.e.~uniform rectangular cell) finite difference \cite{MortonMayers2005} and finite volume \cite{LeVeque} schemes and unstructured finite volume schemes which have been used for layer conservation problems in the context of ice sheet \cite{EgholmNielsen2010} and \cite{Ringleretal2013} ocean modeling.  By considering the dual cell, this finite volume interpretation can also be applied to certain unstructured or structured finite element schemes.  Furthermore, the discrete fluxes $Q_n^{(j,k)}$ can be computed from the cell-averaged (representative) thicknesses, plus other data, by a either a classical (e.g.~Gudonov) or flux-limited scheme \cite{LeVeque}.  Discontinuous Galerkin schemes can also be interpreted as \eqref{eq:fvmassconserve}.

\subsection{Fully-discrete explicit and semi-explicit schemes} \label{subsec:spaceexplicit}  FIXME
...
\end{equation}
the \emph{climate input} during the $n$th time step.  Note the important fact that the climate input $C_n$ is computed by integrating the climate over $\Omega_n$ not $\Omega$.

\subsection{The discrete-time ``retreat loss''}  \label{subsec:retreatloss}  Heuristically, numerical models assert exact discrete conservation when $M_n$ and $C_n$ have computable approximations and
\begin{equation}
	M_n = M_{n-1} + C_n  \label{eq:classicalbalance}
\end{equation}
is true for $n=1,\dots,N$, at least to within rounding error.  However, \eqref{eq:classicalbalance} is not a property which follows from definitions \eqref{eq:totalmassseries} and \eqref{eq:climateseries}.  Instead, define
\begin{equation}
...
\subsection{The discrete-space ``shell error''}  \label{subsec:shellerror}  In the fully-discrete model, with slight abuse of notation, analogous to \eqref{eq:totalmassseries}, \eqref{eq:climateseries}, and \eqref{eq:retreatlossseries}, respectively, we define
	$$M_n = \sum_j u_n^j a_j, \quad C_n = \Delta t\,\sum_{\{j:u_n^j>0\}} F_n^j a_j, \quad \text{ and } \quad R_n = \sum_{\{j:u_n^j=0\}} u_{n-1}^j a_j.$$
Thus by \eqref{eq:fvmassconserve}, and analogous to the continuous-space version \eqref{eq:newbalance}, we compute
\begin{align}
M_n - M_{n-1} &= \sum_{u_n^j>0} (u_n^j - u_{n-1}^j) a_j - R_n \notag \\
   &= - \Delta t\,\sum_{u_n^j>0}\, \sum_{(j,k)\in\mathcal{E}_j} Q_n^{(j,k)} \ell_{(j,k)} + \Delta t\,\sum_{u_n^j>0} F_n^j - R_n \notag \\
   &= - \Delta t\,\sum_{u_n^j>0}\, \sum_{(j,k)\in\mathcal{E}_j} Q_n^{(j,k)} \ell_{(j,k)} + C_n - R_n.  \label{eq:fvnewbalance}
\end{align}
However, though in the continuous-space \eqref{eq:newbalance} we could eliminate the flux integral because $\bQ_n=0$ along $\partial \Omega_n$, the discrete flux sum in \eqref{eq:fvnewbalance} is not zero.

We can use local conservation \eqref{eq:fvlocalconservation} to reduce the discrete flux sum in \eqref{eq:fvnewbalance}.  In fact, [FIXME: interpret as reversing order of summation, using notation for cells which have a given edge?]
\begin{align}
\sum_{u_n^j>0}\, \sum_{(j,k)\in\mathcal{E}_j} Q_n^{(j,k)} \ell_{(j,k)} &= \dots = S_n \label{eq:fvderiveshellerror}
\end{align}
Thus this \emph{shell error} $S_n$ is computed along the finite-volume approximation of the free boundary (shown bold), that is, those edges where one adjacent cell has $u_n>0$ and the other has $u_n=0$.

\begin{figure}[ht]
\begin{center}
\input{fvmesh-shellerror.tikz}
\end{center}
\caption{The \emph{shell error} $S_n$ is computed along those edges (strong line) where $u_n$ changes from positive (labeled with ``$+$'') to zero (labeled ``$0$'').  In the continuous-space case it would be zero.}
\label{fig:fvmesh-shellerror}
\end{figure}

FIXME  thus
\begin{equation}
  M_n = M_{n-1} + C_n - R_n - \Delta t\,S_n \label{eq:fvfinalbalance}
\end{equation}
