...
\section{Well-posedness for the single time-step problem} \label{sec:wellposed}  Our goal in this section is to demonstrate that a variety of different fluxes $\bQ_n$ allow a well-posed weak single time-step problem, variational inequality \eqref{eq:theVI}.  The reader who already believes in well-posed mass conservation equation models with free boundaries can proceed to section \ref{sec:spacediscretized} where fully-discrete numerical schemes are described, so that the \emph{a posterior} analysis of free boundary conservation errors in section \ref{sec:timeseries} can be understood.

Techniques for proving well-posedness, i.e.~existence and uniqueness, of time-independent variational inequalities in Banach spaces are relatively well-established for certain linear and some nonlinear elliptic problems.  We recall these tools, which correspond to monotonic variational inequalities \cite{KinderlehrerStampacchia}, in subsection \ref{subsec:mono}.  Then in the next four subsections we prove existence and uniqueness for these flux cases:
\begin{itemize}
...
\medskip
Corollary III.1.8 of \cite{KinderlehrerStampacchia} now gives the following well-posedness theorem.
...
The observant reader will see a difference in the approaches in theorems \ref{thm:plapadvectwellposed} and \ref{thm:nonlocalwellposed}.  In the former the hyperbolic flux $\bQ_n(u) = \bX(x) u$ was assumed to be regular enough so that an integration-by-parts gave a criterion for coercivity in terms of a norm on the divergence of $\bX$.  In theorem \ref{thm:nonlocalwellposed} we assume only that $\bG$ is integrable, and thus the corresponding integration-by-parts, which would require integrable derivatives $\partial G_i/\partial x_j$, is not tried.  On the other hand, condition \eqref{eq:nonlocaldtcond} on $\Delta t$ is more severe than condition \eqref{eq:plapadvectdtcond}; for instance, \eqref{eq:nonlocaldtcond} involves both at kernel-positivity constant $\delta$ and the Poincar\'e constant.
