...
Our first questions about such models is
  \begin{quote}
...
Our second question is perhaps less obvious, but of substantial importance in modeling practice:
  ...
  \end{enumerate}
  \end{quote}
By considering this question abstractly, we conclude that the answer is ``no,'' in particular in the case where the answer to (i) is ``yes.''  That is, our answer to the brief question ``can a numerical model of a fluid layer governed by \eqref{eq:massconserve} and \eqref{eq:constraint} conserve mass?'' is ``no''.

In the time-semi-discretized case, where each time-step involves a continuous free boundary problem in space, we propose (section \ref{sec:timeseries}) a solution-dependent correction term that can be added to a computable integral of the source $f$ so that a numerical model can exactly (i.e.~up to rounding error) ``balance the books'' in a manner which properly reflects the continuum model of the fluid layer.  In the fully-discretized case we propose an additional solution-dependent correction that is needed to balance the conservation books (section FIXME).  The former correction goes away in the temporal refinement limit (i.e.~$\Delta t\to 0$) and the latter in the joint refinement limit ($\Delta t\to 0$ and $\Delta x\to 0$).
