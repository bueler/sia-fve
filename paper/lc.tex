...
\end{equation}
come from the semi-discretization procedure.  In \eqref{eq:functionalforms}, $\bX\in\RR^d$ is any vector, $v\in\RR$, and $x\in \Omega$.  (Note that we assume $\bQ_n(\bX,v,x)$ is defined for any $v\in\RR$, not just $v\ge 0$.)  According to the particular discretization procedure, $F_n$ ``absorbs'' all the terms, whether evaluated at $t_{n-1}$ or $t_n$, which do not involve the flux $\bq$ evaluated at time $t_n$.
...
\end{align*}
The $\theta=0$ case of \eqref{eq:thetamethod} is the (forward) Euler method, $\theta=1/2$ is the trapezoidal rule, and $\theta=1$ is the backward Euler method.  The fact that $\bQ_n=0$ in the Euler method is completely-acceptable in our weakly-posed theory (Section \ref{sec:weakform}), but we are primarily interested in using some implicitness for stability reasons.

Our theory is not limited to the $\theta$-methods.  In particular, multi-stage one-step schemes like Runge-Kutta can be put in the form of \eqref{eq:semimassconserve}---see Appendix---though hypotheses on the flux (next section) may be harder to check than for the $\theta$-methods above.
...
If $v(x)>0$ on an open subset $S\subset \Omega$, and if $v\in C^1(S)$ then
\begin{equation}
\frac{\partial}{\partial x_i} \bQ_n(\grad v,v,x) \in L_{\operatorname{loc}}^q(S). \label{eq:Qissmooth}
\end{equation}
...
\item the flux is regular (i.e.~continuity in second argument and assumptions \eqref{eq:QisLq} and \eqref{eq:QisLq}),
...
Suppose $u_n\ge 0$ and $v\ge 0$ are sufficiently smooth on $\Omega$ and define $\bQ_n=\bQ_n(\grad u_n,u_n,x)$.  Using decomposition \eqref{eq:omegadecomposition} and integration by parts (divergence theorem) we get
\begin{align*}
...
\end{align*}
Because $u_n$ is continuous, $u_n=0$ along $\Gamma_n^0$ so it follows from \eqref{eq:Qiszero} and equation \eqref{eq:semifixedneumann} that
...
However, by \eqref{eq:Qiszero} we have $\Div \bQ_n=0$ on $\Omega_n^0$ so
    $$\int_{\Omega_n^0} (\Div \bQ_n) (v-u_n) = 0 \ge \int_{\Omega_n^0} \left(F_n - \frac{u_n - u_{n-1}}{\Delta t}\right) (v-u_n),$$
because in fact $u_n=u_{n-1}=0$ and $v-u_n = v \ge 0$ on $\Omega_n^0$, and because $F_n \le 0$ on $\Omega_n^0$.  Almost the same, by \eqref{eq:Qiszero} on $\Omega_n^r$,
    $$\int_{\Omega_n^r} (\Div \bQ_n) (v-u_n) = 0 \ge \int_{\Omega_n^r} \left(F_n - \frac{u_n - u_{n-1}}{\Delta t}\right) (v-u_n)$$
because $u_n=0$ and $v-u_n = v \ge 0$ on $\Omega_n^r$, and because $F_n - (u_n - u_{n-1})/\Delta t = F_n + u_{n-1}/\Delta t \le 0$ on $\Omega_n^r$.  Thus if we return to \eqref{eq:equalitybeforeVI} we have
\begin{align}
...
\begin{align}
\tilde\by &= \by_{n-1} + \tfrac{1}{2} h \bbf(t_n+\tfrac{1}{2}h,\tilde\by), \label{eq:impmid} \\
...
\begin{align}
\tilde\by &= \by_{n-1} + \alpha h \bbf(t_n+\alpha h,\tilde\by), \label{eq:sstabledirk} \\
\by_n &= \by_{n-1} + (1-\alpha) h \bbf(t_n+\alpha h,\tilde\by) + \alpha h \bbf(t_n+h,\by_n). \notag
\end{align}
FIXME: $\bQ_n$ and etc
\end{itemize}
