...
(i)  If \eqref{eq:Qncoercive} holds for $\bQ_n$ then the single time-step weak problem \eqref{eq:theVI} has a unique solution $u\in\mathcal{K}$.
...
\medskip
\begin{theorem}  \label{thm:plapadvectwellposed}  If $\bQ_n$ is given by \eqref{eq:plapadvectflux} with $\eps>0$ and $\bX \in W^{1,\infty}(\Omega;\RR^d)$, and if $p\ge 2$, then for any $\Delta t \le 4 / \|\Div \bX\|_{L^\infty(\Omega)}$, \eqref{eq:theVI} has a unique solution $u\in\mathcal{K}$.
\end{theorem}
