...
\end{center}
\caption{Notation for a completely-unstructured finite volume mesh in $\RR^2$:  Each cell has index $j$, representative thickness $u_n^j$, and area $\omega_j$.  The edge between cell $j$ and cell $k$ is indexed by a pair $(j,k)$, and it has length $\lambda_{(j,k)}$.  The scalar normal flux across this edge, outward from cell $j$, is $Q_n^{(j,k)}$.  Note $Q_n^{(k,j)}=-Q_n^{(j,k)}$.}
...
\end{center}
\caption{In a completely-unstructured mesh, each cell either has $u_n>0$ (labeled with ``$+$'') or $u_n=0$ (labeled ``$0$'').  The shell error is computed along the finite-volume approximation of the free boundary (shown bold), that is, those edges where one adjacent cell has $u_n>0$ and the other has $u_n=0$.}
