...
$$h_t + \Div\bq = f \qquad \to \qquad \frac{H_n - H_{n-1}}{\Delta t} + \Div \bQ_n = F_n$$
  ...
  \item the new equation is a ``single time-step problem''
    ...
    \item[$\circ$] I'll assume:
       \begin{itemize}
       \item[$\diamond$] flux $\bQ_n$ depends on $H_n,\grad H_n,x$
       \item[$\diamond$] source $F_n$ depends on $H_n,x$
       \end{itemize}
    \end{itemize}
  \item details come from time-step scheme
    \begin{itemize}
    \item[$\circ$] forward/backward Euler, trapezoid, RK all o.k.
    \end{itemize}
  \item low regularity of $h(t,x)$ for $x$ near margin means low accuracy expectations for time-step scheme
  \end{itemize}
\end{frame}
...
\begin{frame}{Anyone faced these problems before?}
  ...
  \item yes, of course!
    \begin{itemize}
    \item[$\circ$] generic result: ad hoc schemes for finite volume/difference mass conservation
    \end{itemize}
  \only<2>{
  \item I don't mind ``\texttt{if \dots then \dots}'' in my code, \emph{but} I want to know what mathematical problem it reflects!
  \item we need to define the problem so that practical numerical implementation of boundary conditions is not so \emph{ad hoc}
  }
  \end{itemize}
...
\begin{frame}{Subsets of domain for single time-step problem}
\begin{itemize}
...
\only<1>{
variational inequality for single time-step:
\begin{itemize}
\item is nonlinear because of constraint (i.e~for any $\bQ_n$)
...
\begin{frame}{Terminal slide}
  ...
  \item suggestions:
    ...
    \item[$\circ$]  solve single time-step problem by constrained-Newton method
    \end{itemize}
