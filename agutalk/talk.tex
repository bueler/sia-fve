...
\item I have been agnostic on form of $\bQ_n$
  ...
  \end{itemize}
\item<2-> but form of $\bQ_n$ matters for well-posedness!
\item<3> cases to consider:\small
$$\begin{array}{ll}
  \bQ_n = \bX(x) H_n & \text{\phantom{\Big|}\emph{transported layer}} \\
  \bQ_n = -k \grad H_n & \text{\phantom{\bigg|}\emph{linear diffusion}} \\
  \hspace{-0.5mm}\begin{aligned}
  \bQ_n &= - \grad(H_n^\gamma) \\
        &= - \gamma H_n^{\gamma-1} \grad H_n
  \end{aligned} & \text{\phantom{\Big|}\emph{porous medium}}\phantom{\Big|} \\
...
\begin{frame}{Wrapping up}
  ...
  \item layer flow problems:
    ...
    \item[$\circ$]  conservation modeled by eqn $h_t + \Div\bq = f$ for thickness $h$
    \item[$\circ$]  include constraint $h \ge 0$
    \item[$\circ$]  numerical models discretize in time
    \end{itemize}
  \item \emph{suggestion}:  single time-step problem can be posed weakly as variational inequality, incorporating $h\ge 0$
  \item a limitation of any approach is that exact discrete conservation
    \begin{itemize}
...
%  \begin{itemize}
%  \item to do: FIXME better understanding of well-posedness
%  \item to do: FIXME better numerical schemes
%  \end{itemize}
\end{frame}
