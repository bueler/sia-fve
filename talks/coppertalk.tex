...
\begin{itemize}
\item from now on: steady case $\Delta t \to \infty$ for these slides
...
\item but from now on: restrict to nonsliding SIA
    \begin{itemize}
    \item[$\circ$] other momentum conservation solutions are nontrivial \dots I agree!
      \begin{itemize}
      \item indefinite Stokes or elliptic-ish Blatter-Pattyn or SSA etc.
      \end{itemize}
    \end{itemize}
...
\medskip
nonlinearities of SIA MC:
\begin{enumerate}
\item constrained problem (NCP or VI) is inherently nonlinear
\item $\nu\approx 3$ Glen power
    \begin{itemize}
    \item[$\circ$] in flat bed case, can be transformed to $p$-Laplacian equation
    \end{itemize}
\item bedrock gradient is (un-transformable) additional nonlinearity even if $\nu=1$ (Jouvet \& Bueler 2012)
\end{enumerate}
\end{frame}
