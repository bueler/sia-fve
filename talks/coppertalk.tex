...
\small
\begin{itemize}
\item example: obstacle problem is to find $u(x)$ so that $u''(x) \le 0$, $u(x) \ge \psi(x)$, and either $u=\psi$ or $-u''(x) = 0$
...
\small
\begin{itemize}
\item discretized obstacle problem has $\mathcal{K} = \{z_j \ge \psi(x_j)\}$ and
...
\item constrained optimization $\implies$ NCP $\iff$ VI
  \begin{itemize}
  \item[$\circ$]  i.e.~find minimum of $\mathcal{I}[\bz]$ from $\mathcal{K}$
  \item[$\circ$]  constrained optimization generates symmetric Jacobian/Hessian ($J = \bF' = \mathcal{I}''$)
...
\begin{itemize}
\item FIXME gloss what is in PETSc
\end{itemize}
\end{frame}
...
\item our case:
  ...
  \item[$\circ$]  the mass continuity equation is not (generally) an Euler-Lagrange (variational) equation of a functional
