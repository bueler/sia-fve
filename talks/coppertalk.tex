...
\begin{tikzpicture}[scale=0.6]
  ...
  \draw (5.35,0.35) node {\scriptsize $\square_{j,k-1}$};
  \draw (0.8,3.7) node {\scriptsize $\square_{j-1,k}$};
  \draw (1.0,0.35) node {\scriptsize $\square_{j-1,k-1}$};

  % label center point
  \draw (3,-0.6) node {\footnotesize $x_j$};
  \draw (-0.6,2) node {\footnotesize $y_k$};

  %% indicate coordinates of quadrature points
  %\draw[dotted] (4.5,-0.25) -- (4.5, 0.8);
  %\draw (4.5,-0.7) node {\scriptsize $x_j^0$};
  %\draw[dotted] (4.8,2.5) -- (6.3, 2.5);
  %\draw (6.7,2.5) node {\scriptsize $y_k^0$};
\end{tikzpicture}
\end{center}
\end{frame}
