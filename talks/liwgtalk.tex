...
$$h_t + \Div\bq = f \qquad \to \qquad \frac{H_n - H_{n-1}}{\Delta t} + \Div \bQ_n = F_n$$
  ...
  \bigskip
  \item low regularity of $h(t,x)$ for $x$ near margin
    \begin{itemize}
    \item[$\circ$] low expectations for accuracy there
    \item[$\circ$] ``event detection'' in $\infty$-dimensions is hard
    \end{itemize}
  \end{itemize}
\end{frame}


\begin{frame}{1D examples}
...
\item \alert{the retreat loss $R_n$ is not balanced by the climate}
  ...
  \end{itemize}
\item so a numerical model must track \alert{three} time series:
  \begin{itemize}
  \item[$\circ$] mass at time $t_n$: \qquad $M_n = \int_\Omega H_n(x)\,dx$

  \smallskip
  \item[$\circ$] climate (surface mass balance) over current fluid-covered region:
...
\begin{itemize}
  ...
  \item then
	  \begin{enumerate}
	  \item ``interior condition'' (=PDE) on the set where $H_n>0$:
...
\begin{frame}{Summary}
  ...
  \item claim:  for \emph{any} numerical approach,
    ...
    \item[$\circ$]  exact discrete conservation requires tracking \emph{retreat loss}
      ...
      \item[$\diamond$]  retreat loss only disappears in $\Delta t\to 0$ limit
      \end{itemize}
    \end{itemize}
  \item suggestions:
    \begin{itemize}
